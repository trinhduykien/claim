...
def warmup_api():
    """Warm-up call để load model vào cache sau khi idle lâu (cold start).

    Gọi 1 request nhỏ tới cả 2 model (Kimi + GLM) để tránh cold start chậm/timeout
    ở pipeline chính. Fail im lặng — không ảnh hưởng kết quả.
    """
    if not API_KEY:
        return
    headers = {
    ...
    # Warm-up cả 2 model song song
    models_to_warm = [VISION_MODEL, ANALYSIS_MODEL]
    def _warm_single(model):
        try:
            payload = {
                "model": model,
            ...
            pass  # Warm-up fail không quan trọng
    try:
        with ThreadPoolExecutor(max_workers=2) as ex:
            list(ex.map(_warm_single, models_to_warm))
    except Exception:
        pass
...
def call_analysis_model(messages, max_tokens=16000, timeout=600):
    """Gọi analysis model (GLM-5.2) qua Ollama Cloud API.

    max_tokens mặc định 16000 (tăng từ 12000) để đảm bảo reasoning + content
    đều có đủ token. Khi cold start, GLM sinh reasoning dài hơn bình thường,
    nếu max_tokens quá thấp → toàn bộ token bị dùng cho reasoning → content rỗng.
    """
    headers = {
        "Authorization": f"Bearer {API_KEY}",
        "Content-Type": "application/json"
        ...
        if reasoning.strip():
            ...
            for marker in markers:
                ...
                if idx > last_match_idx:
                    last_match_idx = idx
            if last_match_idx >= 0:
                return {"success": True, "text": reasoning[last_match_idx:][:8000].strip(), "error": ""}
...
def analyze_deduction(claim_data, photo_paths, contract_path):
    ...
    try:
        ...
        # ============================================================
        # WARM-UP: Load model vào cache (tránh cold start chậm/timeout)
        # ============================================================
        _log("Warm-up API (cold start mitigation)...")
        warmup_api()
        _log("Warm-up xong")
        ...
        def run_merge():
            merge_result_box["result"] = call_analysis_model(messages, max_tokens=24000, timeout=600)
        ...
        # ── RETRY 1 LẦN nếu Tier 3 trả về rỗng/thất bại/content rỗng (cold start API) ──
        # Retry với max_tokens gấp đôi (32000) để đảm bảo reasoning + content đều có chỗ
        needs_retry = (
            not analysis_result
            or not analysis_result.get("success")
            or not analysis_result.get("text", "").strip()
        )
        if needs_retry:
            _log("Tier 3 lần 1 rỗng/thất bại — retry lần 2 (max_tokens x2)...")
            retry_box = {"result": None}
            def run_merge_retry():
                retry_box["result"] = call_analysis_model(messages, max_tokens=32000, timeout=600)
            with _status("Đang retry tổng hợp (Tier 3 - lần 2, max_tokens x2)...") as status2:
